...
9. **Posta önbelleği (1.0.45+)** — Sadece ekstreleri silmek bazen **yetmez**: sistem aynı maili «zaten işlendi» saydığı için sync’te yeni PDF oluşmaz (`tekrar` sayısı artar). Kutudaki mesajları baştan işlemek için Ayarlar → **Sistem** → **Posta önbelleğini temizle** (onay `POSTA`). Öğrenilmiş kurallar ve denetim logları **silinmez**. Ardından **Mail ile senkronize et**.
